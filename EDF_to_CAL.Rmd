...
AI_to_CAL <- ttm_all_ai %>%
  mutate(CAL = case_when(AI < band1 ~ "1",
                         AI > band1 & AI < band2  ~ "2",
                         AI > band2 &  AI < band3  ~ "3",
                         AI > band3 &  AI < band4  ~ "4",
                         AI > band4 &  AI < band5  ~ "5",
...
```{r}
COLOR = c('white','#fde725',"#7ad151", "#22a884", "#2a788e", "#414487", "#440154")
...
Labels_CAL = c('0: missing values', '1: least accessible','2','3','4','5','6: most accessible')
...
Global morans I statistic is 0.638 with p value close to 0. The p-value is statistically significant, and the z-score is positive. This reveals that the Access index is not randomly distributed among the features in Camden and the AI is clustered with similar values. 
...
```{r}
CAL_and_PTAL_inner <-CAL_and_PTAL_borough%>%
  filter(inner_outer == "Inner London")
CAL_and_PTAL_outer <-CAL_and_PTAL_borough%>%
  filter(inner_outer == "Outer London")
```
...
map_cal_brent =
  ...
  tm_polygons(col = "CAL",palette=COLOR[2:7],
          showNA = FALSE,
          colorNA = 'white',
          border.alpha = 0,
          title="") +
