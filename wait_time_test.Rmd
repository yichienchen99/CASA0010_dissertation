...
select the points in Enfield / GSS_CODE = E090000010 and City of London / GSS_CODE = E090000001, representing inner and outer london borough. 

```{r}
origin_com <- origin %>%
  filter(gss_code == "E09000010" |  gss_code == "E09000001")
...
```{r}
origin_com_sf <- origin_com %>%
  st_as_sf(.,coords = c("lat", "lon"), crs = 4326)

destin_com_sf <- destin_com %>%
  st_as_sf(.,coords = c("lat", "lon"), crs = 4326)
```
...
tm_shape(origin_com_sf) +
  tm_dots(col = "blue") +
tm_shape(destin_com_sf) +
  tm_dots(col = "red")
```
...
### testing the method for small area first (comden)

```{r}
origin_com_sf$id <- origin_com_sf$id%>%
  as.character(.)

destin_com_sf$id <- destin_com_sf$id%>%
  as.character(.)
```
...
ttm_walk <- expanded_travel_time_matrix(r5r_core = r5r_core,   
                          origins = origin_com_sf,
                          destinations = destin_com_sf,    
                          mode = c("WALK", "TRANSIT"),
...
```{r}
destin_com_sf_bike <- destin_com_sf%>%
  filter(bus_stop == 0)
```
...
ttm_bike <- expanded_travel_time_matrix(r5r_core = r5r_core,   
                          origins = origin_com_sf,
                          destinations = destin_com_sf_bike,    
...
```{r}
origin_com$id<-as.character(origin_com$id)
# add coordinates of destinations to travel time matrix

ttm_all_ai<-ttm_all_ai%>%
  merge(origin_com[,c("id","lon","lat")], by.x = "from_id", by.y = "id")
